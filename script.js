...
(function applyFromQuery() {
	...
	const hasKnown = ["cards", "suits", "ranks", "limit", "sorted"].some((k) => q.has(k));
	if (!hasKnown) return;

	deck.possibleCards = [...deck.deck];

	if (q.get("cards")) {
		const ids = splitParam(q.get("cards"));
		deck.possibleCards = deck.possibleCards.filter((c) => ids.includes(c.id));
	}

	if (q.get("suits")) {
		const suits = splitParam(q.get("suits")).map((s) => s.toLowerCase());
		deck.filter("suit", suits);
	}

	if (q.get("ranks")) {
		const ranks = splitParam(q.get("ranks")).map(mapRank).filter((x) => x !== null);
		deck.filter("rank", ranks);
	}

	if (q.get("sorted")) {
		const dir = String(q.get("sorted")).toLowerCase();
		deck.sort();
		if (dir === "desc") deck.possibleCards.reverse();
	}

	if (q.get("limit")) {
		const n = Math.max(0, parseInt(q.get("limit"), 10) || 0);
		deck.limit(n);
	}
